...
class NetplanApply(utils.NetplanCommand):
    ...
    def command_apply(self, check_generate=True, sync=False, exit_on_error=True, state_dir=None):  # pragma: nocover
        ...
        # Running 'systemctl daemon-reload' will re-trigger the netplan systemd
        # generator. However, daemon-reload is also required to ensure that
        # systemd-networkd reloads its configuration.
